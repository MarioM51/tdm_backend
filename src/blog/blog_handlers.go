...
func (bh BlogHandler) addComment(c *gin.Context) {
	defer apiHelper.HandleApiError(c)

	token := apiHelper.GetRequiredToken(c)
	idBlog := apiHelper.GetIntParam(c, "id")

	commentReceived := bh.getBlogCommentResponse(c)
...
func (bh BlogHandler) addCommentResponse(c *gin.Context) {
	defer apiHelper.HandleApiError(c)

	token := apiHelper.GetRequiredToken(c)

	responseReceived := bh.getBlogCommentResponse(c)
	//id will be generated by yhe BD
	responseReceived.Id = 0
	//responses can`t have stars
	responseReceived.Rating = 0

	//we pass the path variables to response model
	responseReceived.IdBlog = apiHelper.GetIntParam(c, "id")
